...
low-level data structures and high-level collection workflows with stronger
...
- **Deterministic simulations** replay realistic collection and storage workflows so we can
...
- Detect behavioural regressions in query heuristics, constraint solving, and
  collection resolution before they reach downstream users.
...
- `proofs/` already contains Kani harnesses for query, value, and variable-set
  ...
  (`kani::any`, `Inline::new`) and assume/guarantee reasoning that new harnesses
  can reuse.
...
| Collection algebra (`src/collection/*.rs`) | Every representable member join is associative, commutative, and idempotent, while finer covers preserve the total logical join across capacity boundaries; record insertion is idempotent; foundational `Support` is exactly `Cover<SimpleArchive>` and remains invariant across every explicit mapping hop; each target-owned `CollectionDerivation` and explicit `CollectionMapping` is a join homomorphism; substituting an intrinsic mapping-entity id with an extrinsic one does not change meaning; read-only attachment executes no acquisition, joins, or mappings; root `ensure` acquires support without publishing equations, while derived `ensure` publishes only missing `DERIVE` work for one hop; `maintain` additionally reaches a deterministic target size-tier normal form without computing upstream blobs or publishing upstream equations; every successful target `MERGE` or `DERIVE` is persisted even if later work fails; store-level ensure and maintain operations return a fresh `StoreSnapshot`, from which an immutable `CollectionSnapshot` can select frozen support and a realized cover; only Yard/GC removes materializations; COMMIT membership and equation reuse require each producer's target WRITE admission under the frozen proof evidence; a producer signature endorses computation but does not prove its mathematics. | Add bounded record-set generators plus deterministic simulations over resident target results, support-equivalent physical covers, capacity-terminal nodes, and explicitly missing join dependencies. Compare every accepted `Cover<E>` and snapshot-owned logical view with direct source reconstruction, permute record insertion order, inject late failures after successful work, assert that ensure emits no `MERGE`, assert that downstream maintenance never computes upstream artifacts or publishes upstream equations, assert deterministic tiered maintenance, instrument warm paths to prove zero algebra; reject READ-only equation producers, accept delayed WRITE evidence without reinsertion, and check foreign signatures exactly at ingress. Keep direct reconstruction as an offline oracle rather than a production read step. |
...
4. Kick off fuzz targets with `cargo fuzz run <TARGET>` when touching boundary
   code (deserializers, planners, collection sync). Store new corpus inputs in
...
1. Introduce a `cargo fuzz` workspace targeting:
   ...
   - Join-order heuristics to explore combinations of constraint graphs and filter
     predicates.
   - Collection sync workflows by fuzzing unions of commits, merges,
     derivations, missing blobs, and WANTs.
...
1. Model collection replication scenarios with deterministic event queues to
   explore sparse evidence, garbage collection, and concurrent writers.
