...
On a GB10 on 2026-09-15, all explicit CUDA byte/hash gates and three alternating
CPU/CUDA repetitions at each size passed. Median seconds were:

| Rows per input | Build both inputs, CPU / CUDA | Merge, CPU / CUDA |
| ---: | ---: | ---: |
| 65,536 | 0.0470 / 0.0616 | 0.1407 / 0.1427 |
| 250,000 | 0.1809 / 0.1792 | 0.7122 / 0.6824 |
| 1,000,000 | 0.9435 / 0.8199 | 2.9897 / 2.8970 |

The existing Orient and custody processes consumed about nine CPU cores during
these measurements. They establish end-to-end parity and a useful large-build
gain, not an uncontended crossover threshold. The one-million-row gate peaked
at about 952 MiB RSS including recompilation. CPU therefore remains the default;
select CUDA explicitly for measured, externally reserved workloads.
