...
#[test]
fn demand_shallow_full_preserve_exact_wants_and_only_hydrate_selected_roots_without_read_inventory()
{
    let _guard = test_guard();
    let clock = virtual_clock();
    for mode in [
        ...
        runtime.block_on(local.run_until(async {
            ...
            let unaligned = server_store
                ...
                .unwrap();
            // Neither aligned nor unaligned nested words authorize a request
            // without a positive inventory offer from a READ session.
            ...
            for _ in 0..120 {
                ...
                let complete = match mode {
                    ...
                    ReplicationMode::Shallow | ReplicationMode::Full => {
            ...
            assert!(reader.try_local(leaf.raw).is_none());
...
fn full_replication_services_a_fresh_roots_body_while_direct_backlog_grows() {
    ...
    runtime.block_on(local.run_until(async {
        ...
        let body = server_store
            ...
            .unwrap();
        let parent_bytes = body.raw.to_vec();
        ...
        server_store.insert(parent_record).unwrap();
        let mut reader_store = MemoryRepo::default();
        assert_eq!(
            register(&mut reader_store, policy).handle(),
            collection.handle()
        );
        ...
        server.activate_collection(collection.handle());
        reader.activate_collection(collection.handle());
        reader.set_replication(ReplicationMode::Full, [collection.handle()]);
        let mut records = 129;
        let mut offered = 0;
        ...
        assert!(
            offered > 0,
            "positive inventory must progress beside the direct-root backlog"
        );
...
#[test]
fn full_replication_positive_inventory_reaches_a_child_beside_a_large_selection() {
...
fn selection_fairness_case(resident_child: bool) {
    ...
    runtime.block_on(local.run_until(async {
        ...
        let large = server_store
            ...
            .unwrap();
        let small = server_store
            .collection("small selection", policy.clone())
            .unwrap();
        let metadata = server_store
            .put::<SimpleArchive, _>(TribleSet::new().to_blob())
            .unwrap();
        ...
        reader_store
            .collection("large selection", policy.clone())
            .unwrap();
        reader_store.collection("small selection", policy).unwrap();
        ...
        // Place the shared payload between both halves of the large PATCH.
        ...
        for collection in [large.handle(), small.handle()] {
            server.activate_collection(collection);
            reader.activate_collection(collection);
        }
        reader.set_replication(ReplicationMode::Full, [large.handle(), small.handle()]);
        advance(&clock, &mut [&mut server, &mut reader], 4).await;
        let mut reconciler = Reconciler::with_backoff(
            std::time::Duration::from_millis(100),
            std::time::Duration::from_secs(1),
        ...
        assert!(
            ...
            "positive inventory from the small selection must progress beside the large selection"
        );
        assert_eq!(reader.snapshot().unwrap().wants().unwrap().count(), 0);
...
fn full_replication_inventory_learns_a_blob_only_child_arrival_without_semantic_changes() {
    ...
    runtime.block_on(local.run_until(async {
        ...
        let metadata = server_store
            .put::<SimpleArchive, _>(TribleSet::new().to_blob())
            .unwrap();
        // Unknown aligned words are not invitations to issue speculative GETs.
        let mut data_bytes = child.to_vec();
        for ordinal in 0_u64..128 {
            data_bytes.extend_from_slice(blake3::hash(&ordinal.to_be_bytes()).as_bytes());
        }
        let data = server_store
            .put::<UnknownBlob, _>(Bytes::from_source(data_bytes))
            .unwrap();
        ...
        for handle in [collection.handle().raw, data.raw, metadata.raw] {
            let bytes = BlobStoreGet::get::<Bytes, UnknownBlob>(
                &server_store.snapshot().unwrap(),
                Inline::new(handle),
            )
        ...
        assert!(reader.try_local(leaf.raw).is_none());
        let before = triblespace_net::collection_activation::collection_repair_overlay(
        ...
        assert!(
            reader.try_local(leaf.raw).is_some(),
            "positive inventory also exposes the now-reachable resident grandchild"
        );
        assert!(
            offered > 0,
