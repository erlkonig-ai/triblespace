...
mod tests {
    ...
    fn collection_cover_attachment_keeps_every_carrier_lazy() {
        use triblespace_core::collection::{
            descriptor, AdmissionPolicy, CollectionPolicy, CollectionStoreExt,
        };
        use triblespace_core::repo::BlobStorePut;

        let mut store = MemoryRepo::default();
        let descriptor = descriptor::naming::<PortableBM25Blob>(
            "test BM25",
            CollectionPolicy::new(AdmissionPolicy::Open, AdmissionPolicy::Open),
        );
        let collection = store
            .register_collection::<PortableBM25Blob>(descriptor.clone())
            .unwrap();
        let left = golden_index();
