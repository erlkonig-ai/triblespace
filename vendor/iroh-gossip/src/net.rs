...
/// ALPN protocol name
pub const GOSSIP_ALPN: &[u8] = b"/triblespace/gossip/C6858FA15B24B264151DB34DAA9C1964";
...
impl Actor {
    ...
    fn handle_connection(&mut self, peer_id: EndpointId, origin: ConnOrigin, conn: Connection) {
        if self.dialer.is_cancelled(peer_id) {
            // Overflow discarded this dial's control frames. An incoming
            // cross-dial must not turn that failed membership into an active
            // peer before the cancellation reports PeerDisconnected.
            conn.close(1u32.into(), b"gossip dial cancelled");
            return;
        }
        ...
        for event in out {
            ...
            match event {
                OutEvent::SendMessage(peer_id, message) => {
                    // Allow a bounded startup burst proportional to the actual
                    // memberships, not an unbounded history while dialing.
                    let pending_capacity = self.topics.len().saturating_mul(4).max(SEND_QUEUE_CAP);
                    ...
                    match state {
                        PeerState::Active {
                            ...
                            if let Err(error) = enqueue_peer_message(active_send_tx, message).await
                            {
                                // A failed control frame is a failed connection,
                                // never a silently dropped membership transition.
                                // Its task completion reports PeerDisconnected.
                                active_conn.close(1u32.into(), b"gossip writer unavailable");
                                warn!(
                                    peer = %peer_id.fmt_short(),
                                    %error,
                                    "disconnect saturated or closed gossip writer",
                                );
                            }
                        }
                    ...
                    if cancel_pending {
                        warn!(peer = %peer_id.fmt_short(), "cancel saturated pending gossip dial");
                        self.peers.remove(&peer_id);
                        // The cancelled dial reports PeerDisconnected through
                        // the normal dial-completion branch, without blocking
                        // this actor on its own input channel.
...
/// Give a healthy writer one scheduling turn to consume a synchronous burst,
/// but never await capacity whose release depends on a slow remote peer.
async fn enqueue_peer_message(
...
async fn connection_loop(
    ...
    let send_fut = async {
        let result = send_loop.run(queue).await;
        if result.is_err() {
            // Wake the receive half too; join must not hide a failed writer
            // behind an indefinitely open remote receive stream.
            conn.close(1u32.into(), b"gossip send failed");
        }
        result
    }
    .instrument(error_span!("send"));
    let recv_fut = async {
        let result = recv_loop.run().await;
        // Preserve clean directional replacement: simultaneous connections
        // may carry our send and receive legs on opposite connections. FIN
        // ends one leg, not the peer. Once both legs finish, join! completes.
        if result.is_err() {
            conn.close(1u32.into(), b"gossip receive failed");
        }
        result
    }
...
impl Dialer {
    ...
    fn cancel(&self, endpoint: EndpointId) {
        if let Some(cancel) = self.pending_dials.get(&endpoint) {
            cancel.cancel();
        }
    }

    fn is_cancelled(&self, endpoint: EndpointId) -> bool {
        self.pending_dials
            .get(&endpoint)
                ...
                let (endpoint_id, res) = loop {
                    match self.pending.join_next().await {
                        Some(Ok((endpoint_id, res))) => {
                            // Cancellation can race a successful dial already
                            // sitting in the JoinSet. Check it at consumption,
                            // not only inside the connect future.
                            let cancelled = self
                                .pending_dials
                                .remove(&endpoint_id)
...
pub(crate) mod tests {
    ...
    fn queue_test_message() -> ProtoMessage {
        let me = SecretKey::from_bytes(&[1; 32]).public();
        let other = SecretKey::from_bytes(&[2; 32]).public();
        ...
        let message = state
            .handle(
                InEvent::Command(
                    TopicId::from_bytes([71; 32]),
                    ProtoCommand::Join(vec![other]),
                ),
                Instant::now(),
                None,
            )
            .find_map(|event| match event {
                OutEvent::SendMessage(_, message) => Some(message),
                _ => None,
            })
            .unwrap();
        message
    ...
    #[tokio::test]
    async fn saturated_writer_never_waits_for_remote_capacity() {
        let (sender, _receiver) = mpsc::channel(1);
        sender.try_send(queue_test_message()).unwrap();
        let result = timeout(
            Duration::from_millis(100),
            enqueue_peer_message(&sender, queue_test_message()),
        )
        .await
        .expect("full peer queue must not suspend the actor");
        assert!(matches!(result, Err(mpsc::error::TrySendError::Full(_))));
    }

    #[tokio::test]
    async fn healthy_writer_gets_a_scheduling_turn_before_overload() {
        ...
        enqueue_peer_message(&sender, queue_test_message())
            .await
            .expect("writer should drain the initial synchronous burst");
        reader.await.unwrap();
    }

    #[tokio::test]
    async fn closed_writer_is_reported_without_waiting() {
        let (sender, receiver) = mpsc::channel(1);
        drop(receiver);
        assert!(matches!(
            enqueue_peer_message(&sender, queue_test_message()).await,
            Err(mpsc::error::TrySendError::Closed(_))
        ));
    }

    #[tokio::test]
    async fn cancellation_after_dial_task_completed_discards_its_connection() {
        ...
        async fn completed_dial(dialer: &mut Dialer, connection: Connection) {
            let peer = connection.remote_id();
            dialer.pending_dials.insert(peer, CancellationToken::new());
            let (done, ready) = oneshot::channel();
            dialer.pending.spawn(async move {
                done.send(()).unwrap();
                // No yield after the notification: on this current-thread
                // runtime the task returns before the test resumes. The
                // completed result remains unconsumed in the JoinSet.
                (peer, Some(Ok(connection)))
            });
            ready.await.unwrap();
        ...
        let left = endpoint(&network).await;
        let right = endpoint(&network).await;
        let peer = right.id();
        let (connection, remote) = timeout(Duration::from_secs(3), async {
            tokio::join!(left.connect(peer, GOSSIP_ALPN), async {
                right.accept().await.unwrap().accept().unwrap().await
            })
        })
        .await
        .expect("virtual QUIC handshake should finish");
        let connection = connection.unwrap();
        let remote = remote.unwrap();
        let mut dialer = Dialer::new(left.clone());

        // Positive control: the same completed-success path is usable before
        // cancellation, rather than manufacturing an already failed dial.
        completed_dial(&mut dialer, connection.clone()).await;
        let (received_peer, result) = timeout(Duration::from_secs(1), dialer.next_conn())
            .await
            .unwrap();
        assert_eq!(received_peer, peer);
        let accepted = result.unwrap().unwrap();
        assert_eq!(accepted.stable_id(), connection.stable_id());
        ...
        let (received_peer, result) = timeout(Duration::from_secs(1), dialer.next_conn())
            .await
            .expect("cancelled completed dial must be consumed");
        assert_eq!(received_peer, peer);
        assert!(
            result.is_none(),
            "cancelled success must not become an active peer"
        );
        assert!(!dialer.is_pending(peer));
        assert!(
            connection.close_reason().is_some(),
            "discarded connection must be closed"
        );
        timeout(Duration::from_secs(1), remote.closed())
            .await
            .expect("remote must observe cancellation closing the connection");
        left.close().await;
        right.close().await;
    }
